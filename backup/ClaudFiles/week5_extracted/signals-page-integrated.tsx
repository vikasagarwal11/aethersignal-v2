...
export default function SignalExplorerPage() {
  ...
  const fetchSignals = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/v1/signals?${params}`);
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`${API_URL}/api/v1/signals/stats`);
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-400">
                          PRR: {signal.prr.toFixed(1)} | {signal.case_count.toLocaleString()}{" "}
                          cases | AI Confidence:{" "}
          ...
          <CardHeader>
            ...
            {showFilters && (
              <div className="mt-4 p-4 border border-gray-700 rounded-lg bg-gray-800/30">
                ...
                <p className="text-sm text-gray-400">Filter options coming soon...</p>
              </div>
            )}
          </CardHeader>
          <CardContent>
