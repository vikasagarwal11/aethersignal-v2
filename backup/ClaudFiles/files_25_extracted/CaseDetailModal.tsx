...
export function CaseDetailModal({ caseId, drugName, eventName, isOpen, onClose }: CaseDetailModalProps) {
  ...
  const fetchCaseDetails = async () => {
    ...
    try {
      ...
      if (caseId) {
        ...
        url = `/api/v1/cases/by-drug-event?drug=${encodeURIComponent(drugName)}&event=${encodeURIComponent(eventName)}`;
      }
      ...
      if (data) {
        const relatedResponse = await fetch(
          `/api/v1/cases/${data.id}/similar?limit=5`
        );
  ...
  const handleExport = async (format: 'pdf' | 'word') => {
    try {
      const response = await fetch(`/api/v1/cases/${caseData?.id}/export?format=${format}`);
