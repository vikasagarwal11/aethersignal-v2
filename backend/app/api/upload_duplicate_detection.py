...
async def upload_file(
    ...
    """
    Upload file with duplicate handling and session assignment
    
    Args:
        file: Uploaded file
        duplicate_action: 'skip', 'replace', or 'keep' for duplicates
        session_id: Optional UUID of session to assign file to (if None, auto-creates/assigns via trigger)
    
    Returns:
        Upload result with cases created and session_id
    """
    try:
        # Read file content
        ...
        # session_id will be auto-assigned by database trigger if NULL
        upload_record = {
        ...
        # Store organization and user_id for session creation
        # These will be used by the trigger to create organization-scoped sessions
        if organization:
            upload_record["organization"] = organization
        if user_id:
            upload_record["user_id"] = user_id
        
        # If session_id provided, use it; otherwise trigger will auto-create with organization
        if session_id:
        ...
        assigned_session_id = upload_result.data[0].get("session_id")
        ...
        return {
            "status": "success",
            "upload_id": upload_id,
            "session_id": str(assigned_session_id) if assigned_session_id else None,
            "filename": file.filename,
            "file_hash": file_hash,
...
async def get_upload_history(limit: int = 50):
    ...
    try:
        ...
        for upload in result.data:
            ...
            uploads.append({
                **upload,
                "cases_created": cases_result.count if cases_result.count else 0,
                "cases_valid": upload.get("cases_valid", 0),
                "cases_invalid": upload.get("cases_invalid", 0),
                "has_duplicates": duplicate_count.count > 1 if duplicate_count.count else False,
                "duplicate_count": duplicate_count.count if duplicate_count.count else 0
            })
