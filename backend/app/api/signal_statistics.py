...
class SignalDetector:
    ...
    def build_2x2_table(
        ...
        Returns: (a, b, c, d) where:
            ...
            b = Cases with Drug, NO Event
            c = Cases NO Drug, WITH Event
            d = Cases NO Drug, NO Event
        """
        a = b = c = d = 0
        
        for case in all_cases:
            has_drug = self._case_has_drug(case, drug)
            has_event = self._case_has_event(case, event)
    ...
    def _case_has_drug(self, case: Dict, drug: str) -> bool:
        """Check if case involves the drug"""
        drug_name = (case.get('drug_name') or '').strip().lower()
        search_drug = drug.strip().lower()
        return search_drug in drug_name or drug_name in search_drug or drug_name == search_drug
    
    def _case_has_event(self, case: Dict, event: str) -> bool:
        """Check if case involves the event"""
        reaction = (case.get('reaction') or '').strip().lower()
        search_event = event.strip().lower()
        return search_event in reaction or reaction in search_event or reaction == search_event
    ...
    def calculate_prr(
        ...
        # Calculate PRR
        exposed_rate = a / (a + b)
        unexposed_rate = c / (c + d) if (c + d) > 0 else 0
        
        if unexposed_rate == 0:
            prr = float('inf') if a > 0 else 0.0
        else:
            prr = exposed_rate / unexposed_rate
        ...
        try:
            ...
            ci_lower = 0.0
            ci_upper = float('inf') if prr > 0 else 0.0
...
def calculate_signal_statistics(
    ...
    return {
        ...
        'case_count': result.a,
        'prr': {
            'value': round(sanitize_float(result.prr), 2),
            'ci_lower': round(sanitize_float(result.prr_ci_lower), 2),
            'ci_upper': round(sanitize_float(result.prr_ci_upper), 2),
            'is_signal': result.prr_is_signal
        },
        'ror': {
            'value': round(sanitize_float(result.ror), 2),
            'ci_lower': round(sanitize_float(result.ror_ci_lower), 2),
            'ci_upper': round(sanitize_float(result.ror_ci_upper), 2),
            'is_signal': result.ror_is_signal
        },
        'ic': {
            'value': round(sanitize_float(result.ic), 2),
            'ic025': round(sanitize_float(result.ic025), 2),
    ...
    return [
        {
            ...
            'prr_ci': f"[{round(sanitize_float(r.prr_ci_lower), 2)}-{round(sanitize_float(r.prr_ci_upper), 2)}]",
            'ror': round(sanitize_float(r.ror), 2),
            'ror_ci': f"[{round(sanitize_float(r.ror_ci_lower), 2)}-{round(sanitize_float(r.ror_ci_upper), 2)}]",
